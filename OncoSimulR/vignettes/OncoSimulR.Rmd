...
# Additional frequency-dependent fitness examples
__Authors:__ Sara Dorado Alfaro, Miguel Hernández del Valle, 
Álvaro Huertas García, Diego Mañanes Cayero, Alejandro Martín Muñoz.
...
functions at _boxplot_stripchart_pop.R_. 
...
_in a real game of rock paper scissors_ [[8]](#Kerr). It describes the 
relationship between three populations of _Escherichia coli_, that turns out to 
...
```{r message=F}
plot(resultscrs1, show = "genotypes", type = "stacked", legend.out = TRUE)
```
...
plot(resultscrs2a, show = "genotypes", type = "line", legend.out = TRUE)
```
...
plot(resultscrs3[[3]], show = "genotypes", type = "line", cex.lab=1.1,
     las = 1, legend.out = TRUE)
```
...
plot(simulation[[1]], show = "genotypes", type = "line",
     ...
     legend.out = TRUE, legend.pos = -0.22)
...
As is specified in the reference [[2]](#Basanta), 
...
[[10]](#Sartakhti). Here we reproduce some of them.
...
#Plot trajectorie
plot(simulation[[1]], show = "genotypes", legend.out = TRUE)
plot(simulation[[1]], show = "genotypes", type = "line", legend.out = TRUE, legend.pos = -0.215)
...
As specified in [[10]](#Sartakhti) there are two equilibrium points under this
